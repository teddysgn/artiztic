...
use App\Models\CollectionModel as MainModel;
...
class CollectionModel extends AdminModel
{
    ...
    public function listItems($params = null, $option = null){
        ...
        if($option['task'] == 'default-list-items'){
            $result = self::select('id', 'name', 'status', 'picture_profile', 'description')
                            ->where('status', '=', 'active')
                            ->orderBy('id', 'desc')
                            ->get()
                            ->toArray();
        ...
        if($option['task'] == 'default-list-items-in-selectbox'){
            $result = self::select('id', 'name')
                        ->where('status', '=', 'active')
                        ->orderBy('name', 'asc')
                        ->pluck('name', 'id')
    ...
    public function getItem($params = null, $option = null){
        ...
        if($option['task'] == 'get-item'){
            $result = self::select('id', 'name', 'status', 'ordering', 'picture_profile', 'description')
                        ->where('id', '=', $params['id'])
                        ->first();
